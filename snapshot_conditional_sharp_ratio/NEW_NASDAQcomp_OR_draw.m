...
filedir1 = './BCST_NASDAQcomp_CSR/';
filelist1 = dir([filedir1, '*.txt']);
filedir2 = './BCST_NASDAQcomp_CSR_rho/';
...
rf_92_97=load('rf_03_16.txt',','); %risk free
...
for i = (N/2+1):N
    ...
    x_solution = load(['./BCST_NASDAQcomp_CSR/',filename1.name]);
    xt_all(:, i-N/2) = x_solution;
    rho_solution = load(['./BCST_NASDAQcomp_CSR_rho/',filename2.name]);
...
ratio_p_all_yearly = ratio_p_all * (N/13.25)^0.5;
csr_a_yearly = csr_a * (N/13.25)^0.5;
csr_ew_yearly = csr_ew * (N/13.25)^0.5;
csr_index_yearly = csr_index * (N/13.25)^0.5;
