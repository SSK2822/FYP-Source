...
filedir1 = './UPDATE_SP500_CSR/';
filelist1 = dir([filedir1, '*.txt']);
filedir2 = './UPDATE_SP500_CSR_rho/';
...
for i = (N/2+1):N
    ...
    x_solution = load(['./UPDATE_SP500_CSR/',filename1.name]);
    xt_all(:, i-N/2) = x_solution;
    rho_solution = load(['./UPDATE_SP500_CSR_rho/',filename2.name]);
...
csr_a_yearly = csr_a * (N/18)^0.5;
csr_ew_yearly = csr_ew * (N/18)^0.5;
csr_index_yearly = csr_index * (N/18)^0.5;
