...
filedir1 = './BCST_DJIA_CSR/';
filelist1 = dir([filedir1, '*.txt']);
filedir2 = './BCST_DJIA_CSR_rho/';
...
rf_92_97=load('rf_90_16.txt',','); %risk free
...
for i = (N/2+1):N
    ...
    if i-N/2 <= 318
        filename1 = filelist1(i-N/2+363);
        filename2 = filelist2(i-N/2+363);
    end
    if (i-N/2 <= 363) && (i-N/2 >318)
        filename1 = filelist1(i-N/2-318);
        filename2 = filelist2(i-N/2-318);
    end
    if i-N/2 >363
        filename1 = filelist1(i-N/2-363);
        filename2 = filelist2(i-N/2-363);
    ...
    x_solution = load(['./BCST_DJIA_CSR/',filename1.name]);
    xt_all(:, i-N/2) = x_solution;
    rho_solution = load(['./BCST_DJIA_CSR_rho/',filename2.name]);
...
ratio_p_all_yearly = ratio_p_all * (N/26.25)^0.5;
csr_a_yearly = csr_a * (N/26.25)^0.5;
csr_ew_yearly = csr_ew * (N/26.25)^0.5;
csr_index_yearly = csr_index * (N/26.25)^0.5;
