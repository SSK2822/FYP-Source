...
for i = (N/2+1):N %后半期的第i周输出：xt，sor_p,绘图：
    ...
    xt = load(['./BCST_NASDAQ100_SOR/',filename.name]);
    xt_all(:, i-N/2) = xt;
...
sor_yearly = sor * (N/11.5)^0.5;
sor_ew_yearly = sor_ew* (N/11.5)^0.5;
sor_index_yearly = sor_index * (N/11.5)^0.5;
