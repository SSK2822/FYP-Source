...
for i = (N/2+1):N %后半期的第i周输出：xt，sor_p,绘图：
    ...
    xt = load(['./BCST_DJIA_SOR/',filename.name]);
    xt_all(:, i-N/2) = xt;
    ...
    % dsr0 = DSR_p_or(i-N/2, wk_return_d1, xt, rf_00_17);%这里用后验dsr函数
...
sor_yearly = sor * (N/26.25)^0.5;
sor_ew_yearly = sor_ew * (N/26.25)^0.5;
sor_index_yearly = sor_index * (N/26.25)^0.5;
