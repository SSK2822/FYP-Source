...
rt_test = rf_92_97(N/2+1:N);%后半期risk-free
...
for i = (N/2+1):N %后半期的第i周输出：xt，sor_p,绘图：
    ...
    xt = load(['./UPDATE_HSCI_SOR/',filename.name]);
    xt_all(:, i-N/2) = xt;
    ...
    % dsr0 = DSR_p_or(i-N/2, wk_return_d1, xt, rf_92_97);%这里用后验dsr函数
    ...
    dsr0 = DSR_p_or(i-N/2, wk_return_d1, x_ew, rf_92_97);
...
sor_yearly = sor * (N/18)^0.5;
sor_ew_yearly = sor_ew * (N/18)^0.5;
sor_index_yearly = sor_index* (N/18)^0.5;
