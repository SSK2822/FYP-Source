...
filedir = './NEW_DJIA_ESR/';
filelist = dir([filedir, '*.txt']);
rhodir = './NEW_DJIA_ESR_rho/';
...
%{
%CZeSD
x_czesd = load('./DowJones/OptPortfolios_CZeSD_DowJones.txt');
xt_czesd = zeros(M, N);
for ii = 1:N
    if ii>52
...
%KP_SSD
x_kpssd = load('./DowJones/OptPortfolios_KP_SSD_DowJones.txt');
xt_kpssd = zeros(M, N);
for ii = 1:N
    if ii>52
...
%L_SSD
x_lssd = load('./DowJones/OptPortfolios_L_SSD_DowJones.txt');
xt_lssd = zeros(M, N);
for ii = 1:N
    if ii>52
...
%LR_ASSD
x_lrassd = load('./DowJones/OptPortfolios_LR_ASSD_DowJones.txt');
xt_lrassd = zeros(M, N);
for ii = 1:N
    if ii>52
...
%Mean-Variance
x_mv = load('./DowJones/OptPortfolios_MeanVar_DowJones.txt');
xt_mv = zeros(M, N);
for ii = 1:N
    if ii>52
...
%RMZ_SSD
x_rmzssd = load('./DowJones/OptPortfolios_RMZ_SSD_DowJones.txt');
xt_rmzssd = zeros(M, N);
for ii = 1:N
    if ii>52
...
for i = (N/2+1):N 
    ...
    if i-N/2 <= 318
        filename = filelist(i-N/2+363);
    end
    if (i-N/2 <= 363) && (i-N/2 >318)
        filename = filelist(i-N/2-318);
    end
    if i-N/2 >363
        filename = filelist(i-N/2-363);
    end
