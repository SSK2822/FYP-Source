...
for week = (num_weeks/2+1):num_weeks
    ...
    % Calculate maximum drawdown for test data using Calmar_Var_p function
    max_drawdown_test = Calmar_Var_p(week-num_weeks/2, asset_returns, portfolio_weights_all);
    
    % Calculate Calmar ratio for test data using portfolio return and maximum drawdown
    calmar_ratio_test = sum(portfolio_return_all) / (week-num_weeks/2) / max_drawdown_test;
    
    % Save Calmar ratio for current week
    calmar_ratio_p_all(week-num_weeks/2) = calmar_ratio_test;  
end

% Calculate annualized Calmar ratio for test data using different factors depending on the dataset
calmar_ratio_annualized_or = calmar_ratio_p_all* (num_weeks/5.5)^0.5;
calmar_ratio_annualized_update = calmar_ratio_p_all* (num_weeks/18)^0.5;
calmar_ratio_annualized_bsct_DJIA = calmar_ratio_p_all*(num_weeks/26.25)^0.5;
calmar_ratio_annualized_bsct_NASDAQ100 = calmar_ratio_p_all* (num_weeks/11.5)^0.5;
calmar_ratio_annualized_bsct_FTSE100 = calmar_ratio_p_all* (num_weeks/13.83)^0.5;
