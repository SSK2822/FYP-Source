...
output_file = './UPDATE_HDAX_Pain/'; % Output file path
...
for week = (num_weeks/2+1):num_weeks 
    ...
    % Pain ratio for training data
    portfolio_return_train = portfolio_weights' * mean(asset_returns_train, 2) - current_risk_free_rate;
    Pain_ratio_train = portfolio_return_train / Pain_Var_a(week, asset_returns, portfolio_weights);
    Pain_ratio_a_all(week-num_weeks/2) = Pain_ratio_train;
...
% Pain ratio for updated dataset
