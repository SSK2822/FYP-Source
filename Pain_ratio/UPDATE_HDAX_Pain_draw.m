...
% Load data from file
direc = './UPDATE_HDAX_Pain/';
flist = dir([direc, '*.txt']);
asset_prices = load('wk_price_HDAX', ',')';
index_prices = load('b1_HDAX.txt', ','); %index
...
for week = (num_weeks/2+1):num_weeks 
    ...
    portfolio_weights = load(['./UPDATE_HDAX_Pain/',filename.name]);
    ...
    ratio_a_my = My_wk_rt_a_temp / Pain_Var_a(week, asset_returns, portfolio_weights);
...
fontsize_label = 28;
fontsize_legend = 20;
plot(time, ratio_a_all(1:num_weeks/2), '-', 'linewidth', 1.6, 'markersize', marksize, 'color', [0 0.80784 0.81961]);
hold on
plot(time, ratio_p_all(1:num_weeks/2), '--', 'linewidth', 1.6, 'markersize', marksize, 'color', '#B8860B');
hold on
...
saveas(gcf, [filefigures, 'UPDATE_HDAX_PR'], 'eps');
saveas(gcf, [filefigures, 'UPDATE_HDAX_PR'], 'png');