...
rf_90_16=load('rf_02_16.txt',','); %risk free
...
for i = (N/2+1):N %后半期的第i周输出：xt，sor_p：
    ...
    xt = load(['./BCST_FTSE100_OR/',filename.name]);
    xt_all(:, i-N/2) = xt;
...
or_yearly = or * (N/13.83)^0.5;
or_ew_yearly = or_ew * (N/13.83)^0.5;
or_index_yearly = or_index * (N/13.83)^0.5;
