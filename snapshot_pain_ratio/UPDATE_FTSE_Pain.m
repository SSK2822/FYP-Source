...
b1 = load('b1_FTSE.txt',','); 
%index
file1 = './UPDATE_FTSE_Pain/';
rf_00_17 = load('rf_00_17.txt',',');
%risk-free or数据集
...
for i = (N/2+1):N %training output:xt，sor_p：只计算i=470这一周
    i
    wk_return_d1_train = wk_return_d1(:, 1:i-1); 
    %online setting 1-469ex-ante训练数据
    ...
    %miu
    if (miu < rf)
    %if no stock price greater than rf
    ...
    avg_drawdown0 = Pain_Var_p(i-N/2, wk_return_d1, xt_all);
    ratio_my = sum(My_wk_rt) / (i-N/2) / avg_drawdown0;
...
ratio_p_all* (N/18)^0.5; 
%update数据集
ratio_p_all* (N/5.5)^0.5; 
%or 数据集
rr = ratio_a_all* (N/18)^0.5;
rr(1:5)
ratiot_yearly = ratio_p_all*(N/26.25)^0.5; 
%bsct DJIA数据集
ratiot_yearly = ratio_p_all* (N/11.5)^0.5; 
%bsct NASDAQ100数据集
ratiot_yearly = ratio_p_all* (N/13.83)^0.5; 
%bsct FTSE100数据集
ratiot_yearly = ratio_p_all* (N/11.5)^0.5;
