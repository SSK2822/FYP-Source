...
filedir = './NEW_SP500_ESR/';
filelist = dir([filedir, '*.txt']);
rhodir = './NEW_SP500_ESR_rho/';
...
rf_04_16=load('rf_04_16b.txt',','); %risk free
...
%{
%CZeSD
x_czesd = load('./SP500/OptPortfolios_CZeSD_SP500.txt');
xt_czesd = zeros(M, N);
for ii = 1:N
    if ii>52
...
%KP_SSD
x_kpssd = load('./SP500/OptPortfolios_KP_SSD_SP500.txt');
xt_kpssd = zeros(M, N);
for ii = 1:N
    if ii>52
...
%L_SSD
x_lssd = load('./SP500/OptPortfolios_L_SSD_SP500.txt');
xt_lssd = zeros(M, N);
for ii = 1:N
    if ii>52
...
%LR_ASSD
x_lrassd = load('./SP500/OptPortfolios_LR_ASSD_SP500.txt');
xt_lrassd = zeros(M, N);
for ii = 1:N
    if ii>52
...
%Mean-Variance
x_mv = load('./SP500/OptPortfolios_MeanVar_SP500.txt');
xt_mv = zeros(M, N);
for ii = 1:N
    if ii>52
...
%RMZ_SSD
x_rmzssd = load('./SP500/OptPortfolios_RMZ_SSD_SP500.txt');
xt_rmzssd = zeros(M, N);
for ii = 1:N
    if ii>52
...
for i = (N/2+1):N 
    ...
    evar0 = EVaR_p_cal(i, wk_return_d1, xt_all, theta);
    ...
    ratio_ew = sum(ew_wk_rt) / (i-N/2) / EVaR_p_cal(i, wk_return_d1, x_ew_all, theta);
...
for ii = (N/2+1):N 
    ...
    evar0 = EVaR_p_cal(ii, wk_return_d1, xt_mv_all, theta);
...
for ii = (N/2+1):N 
    ...
    My_wk_rt(ii-N/2) = My_wk_rt_temp;
    evar0 = EVaR_p_cal(ii, wk_return_d1, xt_lssd_all, theta);
    ratio_my = sum(My_wk_rt) / (ii-N/2) / evar0;
...
for ii = (N/2+1):N 
    ...
    My_wk_rt(ii-N/2) = My_wk_rt_temp;
    evar0 = EVaR_p_cal(ii, wk_return_d1, xt_lrassd_all, theta);
    ratio_my = sum(My_wk_rt) / (ii-N/2) / evar0;
...
for ii = (N/2+1):N 
    ...
    My_wk_rt(ii-N/2) = My_wk_rt_temp;
    evar0 = EVaR_p_cal(ii, wk_return_d1, xt_rmzssd_all, theta);
    ratio_my = sum(My_wk_rt) / (ii-N/2) / evar0;
...
for ii = (N/2+1):N 
    ...
    My_wk_rt(ii-N/2) = My_wk_rt_temp;
    evar0 = EVaR_p_cal(ii, wk_return_d1, xt_kpssd_all, theta);
    ratio_my = sum(My_wk_rt) / (ii-N/2) / evar0;
...
for ii = (N/2+1):N 
    ...
    My_wk_rt(ii-N/2) = My_wk_rt_temp;
    evar0 = EVaR_p_cal(ii, wk_return_d1, xt_czesd_all, theta);
    ratio_my = sum(My_wk_rt) / (ii-N/2) / evar0;
