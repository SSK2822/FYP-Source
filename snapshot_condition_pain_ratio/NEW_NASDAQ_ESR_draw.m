...
filedir = './NEW_NASDAQ100_ESR/';
filelist = dir([filedir, '*.txt']);
rhodir = './NEW_NASDAQ100_ESR_rho/';
...
%{
%CZeSD
x_czesd = load('./NASDAQ100/OptPortfolios_CZeSD_NASDAQ100.txt');
xt_czesd = zeros(M, N);
for ii = 1:N
    if ii>52
...
%KP_SSD
x_kpssd = load('./NASDAQ100/OptPortfolios_KP_SSD_NASDAQ100.txt');
xt_kpssd = zeros(M, N);
for ii = 1:N
    if ii>52
...
%L_SSD
x_lssd = load('./NASDAQ100/OptPortfolios_L_SSD_NASDAQ100.txt');
xt_lssd = zeros(M, N);
for ii = 1:N
    if ii>52
...
%LR_ASSD
x_lrassd = load('./NASDAQ100/OptPortfolios_LR_ASSD_NASDAQ100.txt');
xt_lrassd = zeros(M, N);
for ii = 1:N
    if ii>52
...
%Mean-Variance
x_mv = load('./NASDAQ100/OptPortfolios_MeanVar_NASDAQ100.txt');
xt_mv = zeros(M, N);
for ii = 1:N
    if ii>52
...
%RMZ_SSD
x_rmzssd = load('./NASDAQ100/OptPortfolios_RMZ_SSD_NASDAQ100.txt');
xt_rmzssd = zeros(M, N);
for ii = 1:N
    if ii>52
