...
filedir_rho = './OR_HSI_CPR_rho/';
filelist_rho = dir([filedir_rho, '*.txt']);
...
for i = (N/2+1):N 
    ...
    xt = load(['./OR_HSI_CPR/',filename.name]);
    xt_all(:,i-N/2) = xt;
    
    filename_rho = filelist_rho(i-N/2);
    rho = load(['./OR_HSI_CPR_rho/',filename_rho.name]);
    rho_all(:,i-N/2) = rho;
    ...
    ratio_a_my = My_wk_rt_a_temp / CDD_Var_a(i, wk_return_d1, xt, rho, theta);
...
ylabel('CPR$_{\rm \bf p}$vs CPR$_{\rm \bf a}$', 'interpreter', 'latex', 'fontsize', fontsize_label)   ;
title('HSI'); % 标题
hold on
