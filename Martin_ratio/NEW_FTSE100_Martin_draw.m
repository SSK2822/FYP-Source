...
risk_free_rate=load('rf_02_16.txt',','); %risk free
...
%CZeSD
x_czesd = load('../data/BCST_Solutions/FTSE100/OptPortfolios_CZeSD_FTSE100.txt');
...
%KP_SSD
x_kpssd = load('../data/BCST_Solutions/FTSE100/OptPortfolios_KP_SSD_FTSE100.txt');
...
%L_SSD
x_lssd = load('../data/BCST_Solutions/FTSE100/OptPortfolios_L_SSD_FTSE100.txt');
...
%LR_ASSD
x_lrassd = load('../data/BCST_Solutions/FTSE100/OptPortfolios_LR_ASSD_FTSE100.txt');
...
%MV
x_mv = load('../data/BCST_Solutions/FTSE100/OptPortfolios_MeanVar_FTSE100.txt');
...
%RMZ_SSD
x_rmzssd = load('../data/BCST_Solutions/FTSE100/OptPortfolios_RMZ_SSD_FTSE100.txt');
...
for week = (num_weeks/2+1):num_weeks 
    ...
    ratio_a_my = My_wk_rt_a_temp / Martin_Var_a(week, asset_returns, portfolio);
...
for weeks = (num_weeks/2+1):num_weeks 
    ...
    ratio0 = Martin_Var_p(weeks-num_weeks/2, asset_returns, portfolio_mv_all);
...
for weeks = (num_weeks/2+1):num_weeks 
    ...
    Curr_week_rt(weeks-num_weeks/2) = Curr_week_temp;
    ratio0 = Martin_Var_p(weeks-num_weeks/2, asset_returns, portfolio_lssd_all);
    ratio_my = sum(Curr_week_rt) / (weeks-num_weeks/2) / ratio0;
...
for weeks = (num_weeks/2+1):num_weeks 
    ...
    Curr_week_rt(weeks-num_weeks/2) = Curr_week_temp;
    ratio0 = Martin_Var_p(weeks-num_weeks/2, asset_returns, portfolio_lrassd_all);
    ratio_my = sum(Curr_week_rt) / (weeks-num_weeks/2) / ratio0;
...
for weeks = (num_weeks/2+1):num_weeks 
    ...
    Curr_week_rt(weeks-num_weeks/2) = Curr_week_temp;
    ratio0 = Martin_Var_p(weeks-num_weeks/2, asset_returns, portfolio_rmzssd_all);
    ratio_my = sum(Curr_week_rt) / (weeks-num_weeks/2) / ratio0;
...
for weeks = (num_weeks/2+1):num_weeks 
    ...
    Curr_week_rt(weeks-num_weeks/2) = Curr_week_temp;
    ratio0 = Martin_Var_p(weeks-num_weeks/2, asset_returns, portfolio_kpssd_all);
    ratio_my = sum(Curr_week_rt) / (weeks-num_weeks/2) / ratio0;
...
for weeks = (num_weeks/2+1):num_weeks 
    ...
    Curr_week_rt(weeks-num_weeks/2) = Curr_week_temp;
    ratio0 = Martin_Var_p(weeks-num_weeks/2, asset_returns, portfolio_czesd_all);
    ratio_my = sum(Curr_week_rt) / (weeks-num_weeks/2) / ratio0;
...
ylabel('MR$_{\rm \bf p}$vs MR$_{\rm \bf a}$', 'interpreter', 'latex', 'fontsize', fontsize_label)   ;
title('FTSE 100');
hold on
...
saveas(gcf, [filefigures, 'NEW_FTSE_MR'], 'eps');
saveas(gcf, [filefigures, 'NEW_FTSE_MR'], 'png');
