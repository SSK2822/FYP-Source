...
output_file = './UPDATE_HDAX_Martin/'; % Output file path
...
for week = (num_weeks/2+1):num_weeks 
    ...
    % Martin ratio for training data
    portfolio_return_train = portfolio_weights' * mean(asset_returns_train, 2) - current_risk_free_rate;
    Martin_ratio_train = portfolio_return_train / Martin_Var_a(week, asset_returns, portfolio_weights);
    Martin_ratio_a_all(week-num_weeks/2) = Martin_ratio_train;
...
% Martin ratio for updated dataset
Martin_ratio_update = Martin_ratio_p_all* (num_weeks/18)^0.5; 
